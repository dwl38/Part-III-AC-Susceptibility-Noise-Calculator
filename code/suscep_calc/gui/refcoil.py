#========================================================================================================================
# This file is for the sub-menu for the reference coil.
#========================================================================================================================
...
class Refcoil(GUIElement):
    ...
    def __init__(self, parent, num_validator, pos_num_validator, pos_int_validator, PROGRAM_DIRECTORY):
        ...
        self.frame = ttk.LabelFrame(parent, text='Reference Coil Parameters', padding=10)
    ...
    def get_configuration(self, prev: dict = None) -> dict:
        output = dict()
        output['refcoil_radius'] = self.radius_entry_variable.get()
        output['refcoil_radius_units'] = self.radius_units_variable.get()
        output['refcoil_length'] = self.length_entry_variable.get()
        output['refcoil_length_units'] = self.length_units_variable.get()
        output['refcoil_nturns'] = self.nturns_entry_variable.get()
        output['refcoil_thickness'] = self.thickness_entry_variable.get()
        output['refcoil_thickness_units'] = self.thickness_units_variable.get()
        output['refcoil_mat'] = self.material_entry_variable.get()
        output['refcoil_offset'] = self.offset_entry_variable.get()
        output['refcoil_offset_units'] = self.offset_units_variable.get()
    ...
    def set_configuration(self, config: dict) -> Tuple[int, str]:
        ...
        if 'refcoil_radius' in config and 'refcoil_radius_units' in config:
            var = config['refcoil_radius']
            unit = config['refcoil_radius_units']
            if is_nonneg_real_number(var) and unit in LENGTH_UNITS:
                ...
                result = 1
                err_str += 'Refcoil radius invalid\n'
        else:
            self.radius_entry_variable.set('0.0')
            self.radius_units_variable.set('m')
            result = 1
            err_str += 'Missing refcoil radius\n'

        if 'refcoil_length' in config and 'refcoil_length_units' in config:
            var = config['refcoil_length']
            unit = config['refcoil_length_units']
            if is_nonneg_real_number(var) and unit in LENGTH_UNITS:
                ...
                result = 1
                err_str += 'Refcoil length invalid\n'
        else:
            self.length_entry_variable.set('0.0')
            self.length_units_variable.set('m')
            result = 1
            err_str += 'Missing refcoil length\n'
            
        if 'refcoil_nturns' in config:
            var = config['refcoil_nturns']
            if is_nonneg_int(var):
                self.nturns_entry_variable.set(var)
            else:
                self.nturns_entry_variable.set('0')
                result = 1
                err_str += 'Refcoil number of turns invalid\n'
        else:
            self.nturns_entry_variable.set('0')
            result = 1
            err_str += 'Missing refcoil number of turns\n'
            
        if 'refcoil_thickness' in config and 'refcoil_thickness_units' in config:
            var = config['refcoil_thickness']
            unit = config['refcoil_thickness_units']
            if is_nonneg_real_number(var) and unit in LENGTH_UNITS:
                ...
                result = 1
                err_str += 'Refcoil thickness invalid\n'
        else:
            self.thickness_entry_variable.set('0.0')
            self.thickness_units_variable.set('mm')
            result = 1
            err_str += 'Missing refcoil thickness\n'
            
        if 'refcoil_mat' in config:
            var = config['refcoil_mat']
            if os.path.isfile(var):
                self.material_entry_variable.set(var)
            else:
                self.material_entry_variable.set('')
                result = 1
                err_str += 'Refcoil material invalid\n'
        else:
            self.material_entry_variable.set('')
            result = 1
            err_str += 'Missing refcoil material\n'
        
        if 'refcoil_offset' in config and 'refcoil_offset_units' in config:
            var = config['refcoil_offset']
            unit = config['refcoil_offset_units']
            if is_real_number(var) and unit in LENGTH_UNITS:
                ...
                result = 1
                err_str += 'Refcoil offset invalid\n'
        else:
            self.offset_entry_variable.set('0.0')
            self.offset_units_variable.set('m')
            result = 1
            err_str += 'Missing refcoil offset\n'
